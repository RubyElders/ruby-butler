...
impl BundlerRuntimeDetector {
    ...
    pub fn discover(start_dir: &Path) -> std::io::Result<Option<BundlerRuntime>> {
        debug!("Starting Bundler discovery from directory: {}", start_dir.display());
        ...
        info!("No Bundler project found starting from: {}", start_dir.display());
        Ok(None)
    }
    ...
    pub fn discover_from_cwd() -> std::io::Result<Option<BundlerRuntime>> {
        ...
        debug!("Discovering Bundler runtime from current working directory: {}", cwd.display());
        Self::discover(&cwd)
    }
}
...
"#;
        ...
        assert!(result.is_some());
        let bundler_runtime = result.unwrap();
        assert_eq!(bundler_runtime.ruby_version(), Some(semver::Version::parse("3.2.1").unwrap()));
