...
fn bundler_runtime_provides_correct_paths_for_configured_project() -> io::Result<()> {
    ...
    assert_eq!(bundler_runtime.vendor_dir(), project_dir.join(".rb").join("vendor").join("bundler"));
    assert_eq!(bundler_runtime.bin_dir(), bundler_runtime.vendor_dir().join("bin"));
...
fn bundler_runtime_detects_ruby_version_from_ruby_version_file() -> io::Result<()> {
    ...
    let bundler_runtime = BundlerRuntime::new(&project_dir);
    assert_eq!(bundler_runtime.ruby_version(), Some(Version::parse("3.2.5").unwrap()));
...
"#;
    ...
    let bundler_runtime = BundlerRuntime::new(&project_dir);
    assert_eq!(bundler_runtime.ruby_version(), Some(Version::parse("3.1.2").unwrap()));
...
"#;
    ...
    let bundler_runtime = result.unwrap();
    assert_eq!(bundler_runtime.ruby_version(), Some(Version::parse("3.3.1").unwrap()));
...
"#;
    ...
    assert_eq!(bundler_runtime.ruby_version(), Some(Version::parse("3.2.3").unwrap()));
