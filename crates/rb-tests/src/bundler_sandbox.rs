...
impl BundlerSandbox {
    ...
    pub fn add_file<S: AsRef<str>>(&self, path: S, contents: impl AsRef<[u8]>) -> io::Result<PathBuf> {
    ...
    pub fn add_gemfile<S: AsRef<str>>(&self, dir_path: Option<S>, contents: Option<&str>) -> io::Result<PathBuf> {
    ...
    pub fn add_bundler_project<S: AsRef<str>>(&self, project_name: S, configured: bool) -> io::Result<PathBuf> {
    ...
    pub fn add_complex_project(&self) -> io::Result<(PathBuf, PathBuf, PathBuf)> {
        ...
        fs::write(&sub_gemfile, "source 'https://rubygems.org'\n\ngem 'engine-specific-gem'\n")?;
        
        // Deep nested directory without Gemfile
        let deep_dir = subproject_dir.join("app").join("controllers").join("concerns");
...
mod tests {
    ...
    fn add_complex_project_creates_nested_gemfiles() -> io::Result<()> {
        ...
        assert!(root_project.join(".rb").join("vendor").join("bundler").exists());
