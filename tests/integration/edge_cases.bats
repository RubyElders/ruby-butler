...
EOF
    ...
    if [ "$status" -eq 0 ]; then
        output_contains "3.1.0"  # Fallback to available
    else
...
@test "handles corrupted Ruby installation gracefully" {
    # Create Ruby installation with bin directory but no executable
    local ruby_dir="$RUBIES_DIR/ruby-3.2.5"
...
@test "handles version resolution edge cases" {
    # Create multiple Ruby versions with complex version numbers
    create_ruby_installation "3.2.0-preview1"
    create_ruby_installation "3.2.0-rc1" 
    create_ruby_installation "3.2.0"
    create_ruby_installation "3.2.1"
    
    run_rb_success runtime
    
    # Should properly sort and select latest stable version
    output_contains "Ruby Environment Survey"
    # Should prefer 3.2.1 over preview/rc versions
    output_contains "(3.2.1)"
}
